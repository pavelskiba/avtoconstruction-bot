...
3. Проверка подписки через Telegram API (`getChatMember`). Не подписан —
   ...
   бесплатное вводное видео, заглушку гайда и две кнопки — «Оплатить»
   (ссылка на созданный через API ЮKassa платёж, 990 ₽) и «Информация о
   курсе» (лендинг).
4. Бот сам создаёт платёж через ЮKassa API (`payments.create_payment`) с
   `metadata.telegram_user_id` — это позволяет потом связать оплату с
   конкретным пользователем Telegram.
...
`new` → `waiting_subscribe` → `guide_sent` → `paid`
